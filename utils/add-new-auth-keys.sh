...
echo ""
echo "To enable asymmetric key pair, the following should be enabled in docker-compose.yaml:"
echo ""
...
# Uncomment new auth configuration in docker-compose.yaml
...
if grep -q '^[ ]*GOTRUE_JWT_KEYS:' docker-compose.yaml && \
   grep -q '^[ ]*API_JWT_JWKS:' docker-compose.yaml && \
   grep -q '^[ ]*JWT_JWKS:' docker-compose.yaml; then
    echo "Done."
else
    echo "Warning: could not edit docker-compose.yaml. Uncomment auth configuration manually."
fi